...
public class CaesarCipher
{
    ...
    public String decrypt (String cipherText)
    {
        ...
        int index; // loop control 
        char plainCh, plainUpper, cipherCh, cipherUpper; 
        int cipherPos; // position of cipherCh in ALPHABET
        ...
        for (index = 0; index < length; index++) { 
            ...
            if (cipherPos == -1) { 
                // cipherCh is not in ALPHABET
                cipherCh = cipherCh; 
            }
            else {
                // cipherCh is in ALPHABET
                cipherUpper = decodeMap.charAt(cipherPos); 
                if (Character.isUpperCase(cipherCh)) 
                    cipherCh = cipherUpper; 
                else 
                    cipherCh = Character.toLowerCase(cipherUpper); 
            }
            plainText = plainText + cipherCh;
        }
        return plainText; 
    }
}